<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="<?php echo e(route('classe.create')); ?>" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
...
<?php if($count): ?>
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded"><?php echo e($count); ?> classes trouvées</span></div>
<?php endif; ?>
    <div class="flex  -mx-3 mb-0  space-x-4 space-x-reverse" style="padding:8px;">
        <?php if(optional(Auth()->user()->personnel)->etablissement_id === null): ?>
        <div class="w-full px-7">
            ...
            <select id="selectedEtablissement" wire:model="selectedEtablissement" name="selectedEtablissement" wire:change="$refresh" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" required autofocus>
                ...
                <option value="<?php echo e($etablissement->id); ?>"><?php echo e($etablissement->nom); ?></option>
        ...
        <div class="w-full px-7">
            ...
            <select id="selectedClasseAnnee" wire:model="selectedClasseAnnee" name="selectedClasseAnnee" wire:change="$refresh" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" required autofocus>
                <option value="">Choisissez une année académique</option>
                <?php $__currentLoopData = $annee_academique; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $annee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="w-full bg-transparent rounded-lg shadow-xs p-0 flex flex-wrap">
        ...
        <div class="sm:w-1/3 w-full p-4">
            <div class="rounded-lg shadow-md p-4 bg-white2 ">
                <h1 class="font-bold text-lg"><i class="fa-solid fa-building-user" style="color:green;"></i> <?php echo e($classe->libelle); ?></h1>
                <hr class="my-2" />
                <div class="flex mb-4 items-center">
                    <div class="px-4 flex-1">
                        <h3 class="text-sm py-1"><i class="text-green-600 fa fa-building"></i>&nbsp;EFPT : <span class="font-bold"><?php echo e($classe->etablissement->nom ?? ' - '); ?></span></h3>
                        <h3 class="text-sm py-1"><i class="text-green-600 fa fa-star"></i>&nbsp;Filière : <span class="font-bold"><?php echo e($classe->niveau_etude->metier->filiere->nom ?? ' - '); ?></span></h3>

                        <h3 class="text-sm py-1"><i class="text-green-600 fa fa-star"></i>&nbsp;Niveau Etude : <span class="font-bold"><?php echo e($classe->niveau_etude->nom ?? ' - '); ?></span></h3>

                        <h3 class="text-sm py-1"><i class=" text-green-600 fas fa-window-maximize"></i></i>&nbsp;Modalite : <span class="font-bold"><?php echo e($classe->modalite ?? ' - '); ?></span></h3>
                        <h3 class="text-sm py-1"><i class="text-green-600 fa fa-tag"></i>&nbsp;Année académique : <span class="font-bold"><?php echo e($classe->annee_academique->annee1 . ' - ' . $classe->annee_academique->annee2  ?? ' - '); ?></span></h3>
                    </div>
                    <div clas s="px-4 flex items-end">
                        <div class="flex items-center <?php echo e($classe->statut ? 'bg-green-100' : 'bg-red-100'); ?> px-4 py-1 rounded-lg">
                            <i class="fa fa-circle <?php echo e($classe->statut ? 'text-green-600' : 'text-red-600'); ?> text-xs pr-3"></i>
                            <h3><?php echo e($classe->statut ? 'Lancée' : 'en attente'); ?></h3>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="mt-4">
                    <div class="flex justify-between items-center mt-3">
                        <a href="<?php echo e(route('classe.edit',$classe->id)); ?>" class="flex items-center px-1 rounded-md py-1 border flex text-orange-600 text-sm text-center bg-white border-orange-600 hover:bg-orange-600 hover:text-white">
                            <i class="fa fa-edit"></i>
                            <span class="mx-2">Modifier</span>
                        </a>
                        <?php if(!$classe->statut): ?>
                        <a href="<?php echo e(route('classe.validate', $classe->id)); ?>" class="flex items-center px-1 rounded-md py-1 border flex text-purple-600 text-sm text-center bg-white border-purple-600 hover:bg-purple-600 hover:text-white">
                            <i class="fa fa-check"></i>
                            <span class="mx-2">Lancer</span>
                        </a>
                        <?php endif; ?>
                        <a href="<?php echo e(route('classe.show',$classe->id)); ?>" class="flex items-center px-1 rounded-md py-1 border flex text-green-600 text-sm text-center bg-white border-green-600 hover:bg-green-600 hover:text-white">
                            <i class="fa fa-eye"></i>
...
<?php /**PATH /var/www/html/amie-fpt/resources/views/livewire/classe/liste-classeFA.blade.php ENDPATH**/ ?>