<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            <a href="#" style="visibility: hidden;" class="mx-2 px-5 rounded-md py-0 flex text-white text-xs font-bold text-center shadow-md bg-orange-400 items-center">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 m-2">

        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $les_metiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nom => $niveaux): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="w-full h-auto bg-white rounded-md shadow-md">
            <div class="p-3">
                <div class="font-bold text-lg mb-2 bg-gray-200 p-2 rounded"><?php echo e(Str::limit($nom ?? '-', 32)); ?></div>
            </div>
            <div class="px-3 py-2">
                <ul class="max-h-48 overflow-y-auto">
                    ...
                    <?php
                    $status = $this->checkStatusFormation($niv->id, $etablissement->id);
                    $statusClasses = $status ? 'text-green-800 bg-green-200' : 'text-orange-800 bg-orange-200';
                    $label =  $status ? "Accepté" : "Refusé"
                    ?>
                    <span class="inline-block px-2 py-1 text-xs font-semibold <?php echo e($statusClasses); ?> rounded-full"><?php echo e($label); ?>

                    </span>
                    <li class="border-b p-2 font-normal mb-1">
                        <i class="fa fa-circle text-xs me-2"></i><?php echo e($niv->nom ?? '-'); ?>

                    </li>
                    <a href="<?php echo e(route('program.validate', ['id' => $niv->id, 'decision' => 'false', 'idEtablissement' => $etablissement->id])); ?>" title="Refuser le programme de formation" class="rounded-full px-1 py-1 text-white text-xs font-bold text-center bg-orange-500  hover:bg-orange-700">
                        <span class="mx-1"><i class="fa fa-times"></i></span>
                    </a>
                    <a href="<?php echo e(route('program.validate', ['id' => $niv->id, 'decision' => 'true', 'idEtablissement' => $etablissement->id])); ?>" title="Valider le programme de formation" class="rounded-full ml-1 px-1 py-1 text-white text-xs font-bold text-center bg-blue-500  hover:bg-blue-700">
                        <span class="mx-1"><i class="fa fa-check"></i></span>
                    </a>
                    <a href="<?php echo e(route('program.logic.remove', ['id' => $niv->id, 'idEtablissement' => $etablissement->id])); ?>" title="Supprimer le programme de formation" class="rounded-full ml-1 px-1 py-1 text-white text-xs font-bold text-center bg-red-500  hover:bg-red-700">
                        <span class="mx-1"><i class="fa fa-trash"></i></span>
                    </a>
...
</div><?php /**PATH C:\wamp64\www\AMIE-FPT\resources\views/livewire/niveauetudeetablissement/view-program-formation.blade.php ENDPATH**/ ?>