            ...
            <div class="flex flex-col min-w-0">
                @if(Auth()->user()->personnel && Auth()->user()->personnel->etablissement)
                    <h1 class="text-lg font-bold truncate md:text-xl text-primary-600">
                        {{ Auth()->user()->personnel->etablissement->nom }}
                    </h1>
                @endif
                ...
                <div class="text-sm ">
                    ...
                    <span class="ml-2 text-xs text-gray-500">{{ $dateActuelle }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-3">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open"
                    ...
                    <img class="w-16 h-16 rounded-full" src="{{ asset('backofficeAssets/build/images/imagepgi.svg') }}" alt="Profil">
                </button>
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    @if(!auth()->user()->hasRole(config('constants.roles.superadmin')) && !auth()->user()->hasRole(config('constants.roles.ia')) && auth()->user()->can('visualiser_etablissement_info'))
    ...
    <div class="border-b md:hidden bg-sp-green" x-show="isMobileMainMenuOpen"
        ...
        <div>
            ...
            @if(auth()->user()->can('visualiser_classe_matiere'))
                <a class="element_sidebar {{ request()->is('classe*') ? 'element_sidebar_acitf' : '' }}" href="{{route('classe.index')}}">
                    <span class="text-left">
...
<div class="bg-gray-100 border-x border-2-maquette-gris rounded shadow-inner relative -top-2 overflow-hidden transition-all max-h-0 duration-700" style="" x-ref="container1" x-bind:style="open ? 'max-height: ' + $refs.container1.scrollHeight + 'px' : 'border:none'">
      <a class="element_sidebar {{ request()->is('inscription*') ? 'element_sidebar_acitf' : '' }} {{ request()->is('evaluation*') ? 'element_sidebar_acitf' : '' }}" href="{{route('inscription.index')}}">
                ...
                <a class="element_sidebar" href="{{route('competence.manage.index')}}">
                    <span class="text-left">
                    <i class="fas fa-user"></i>
                    </span>
                    <span class="mx-4 text-base font-normal">
