<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification de la competence') }}
        </h2>
    </x-slot>
    {{--le bouton pour le retour en arrière  --}}
    ...
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            ...
            <a href="{{route('competence.index')}}"  class="text-maquette">Référentiel</a>
              <span class="mx-2 text-maquette">/</span>
        <p> <a href="{{route('competence.index')}}" >Compétence  </a>
            
            <span class="mx-2 text-maquette">/</span>
        </p>

        <p class="text-first-orange">Modification de la compétence</p>
        </p>
    ...
    <div class="rounded-sm w-full p-4 mt-2">
        ...
        <div class="rounded-sm w-full">
            <div class="w-full mx-auto max-w-6xl">
                <form action="{{ route('competence.update', $competence->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    @csrf
                    ...
                    <div class="flex flex-wrap w-full justify-evenly">
                        <div class="flex-grow mb-4 mr-2">
                            ...
                            <x-input id="code" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="code" value="{{ $competence->code }}" required autofocus autocomplete="code" />
                            @error('code')
                        ...
                        <div class="flex-grow mb-4 mr-2">
                            ...
                            <x-input id="nom" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="nom" value="{{ $competence->nom }}" required autofocus autocomplete="nom" />
                            @error('nom')
                                <span class="text-xs text-red-500">
                                    {{$message}}
                                </span>
                        ...
                        <!--div class="flex-grow mb-4 mr-2">
                            ...
                            <x-input id="coef" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="coef" value="{{ $competence->coef }}" required autofocus autocomplete="coef" />
                            @error('coef')
                                <span class="text-xs text-red-500">
                                    {{$message}}
                                </span>
                    ...
                    @livewire('parametrage.Competence.EditCompetence')
                    ...
                    <div class="mb-4 mx-auto w-full">
                        ...
                        <textarea value="{{ $competence->description }}" class="enlever_shadow shadow appearance-none border-2 border-gray-400 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:border-first-orange" id="description" name="description" required rows="4" placeholder="Description">{{$competence->description}}</textarea>
                        @error('description')
                            <span class="text-xs text-red-500">
                                {{$message}}
                            </span>
