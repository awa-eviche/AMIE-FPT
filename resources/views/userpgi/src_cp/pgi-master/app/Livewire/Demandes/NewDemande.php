...
class NewDemande extends Component
{
    ...
    public function soumettre()
    {
        ...
        if ($validator->fails()) {
            ...
            if ($validator->errors()->hasAny(['nom', 'prenom', 'email', "adresse"])) {
                $this->livewireStep = 1;
            } else if ($validator->errors()->hasAny(["email_etablissement", "nom_etablissement", "statut_juridique", "statut", "type", "commune_id", "date_creation"])) {
                $this->livewireStep = 2;
    ...
    public function enregistrerBrouillon()
    {
        ...
        if ($validator->fails()) {
            ...
            if ($validator->errors()->hasAny(['nom', 'prenom', 'email', 'adresse'])) {
                $this->livewireStep = 1;
            } else if ($validator->errors()->hasAny(["email_etablissement", "nom_etablissement", "commune_id", "date_creation", "statut_juridique", "statut", "type"])) {
                $this->livewireStep = 2;
            } else if ($validator->errors()->hasAny([])) {
                $this->livewireStep = 4;
            }
