...
class WorkflowTools {
    ...
    public function __construct(private UserRepository $userRepository, private NotificationService $notificationService) {
    }
    /**
    ...
    public function next(Demande $demande, $user_id) {
        ...
        try {
            ...
            $this->notificationService->sendNotification($usersToNotify, $typeNotification->message, EnumTypeNotification::SYSTEME, $typeNotification->action, ["entity"=>$demande, "route"=> "demande.show"]);
        } catch (\Throwable $th) {
            return true;
        }
        // Pour le promoteur, c'est clair que l'on ne peut pas passer uniquement par profil. Pour cela, je pense ajouter un profil propriétaire.
