...
class DepartementController extends Controller
{
  ...
  public function __construct(
        ...
        if ($lastDep) {
            if (Region::where('id', $request->region_id)->exists()) {
                ...
                return redirect()->route('departement.index')
                         ->with('message','Département créé avec succès.');
            } else {
                return redirect()->route('departement.index')
                         ->with('message','Département sélectionné n\'existe pas.');
            }
        } else {
            return redirect()->route('departement.index')
                         ->with('message','Aucun enregistrement n\'a été effectué.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('departement.index')
                         ->with('success', 'département mise à jour avec succès.');
    }
