<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Modification du niveau d'étude") }}
        </h2>
    </x-slot>
    ...
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            <a href="{{route('niveauetude.index')}}"  class="text-maquette">Accueil</a>
              <span class="mx-2 text-maquette">/</span>
            <a href="{{route('niveauetude.index')}}"  class="text-maquette">Référentiel</a>
              <span class="mx-2 text-maquette">/</span>
        <p> <a href="{{route('niveauetude.index')}}" >Niveau Etude  </a>
    ...
    <div class="rounded-sm w-full p-4 mt-2">
        ...
        <div class="rounded-sm w-full">
            <div class="w-full mx-auto max-w-6xl">
                <form action="{{ route('niveauetude.update', $niveauetude->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    @csrf
                    ...
                    <div class="flex flex-wrap w-full justify-evenly">
                        <div class="flex-grow mb-4 mr-2">
                            ...
                            <x-input id="code" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="code" value="{{ $niveauetude->code }}" required autofocus autocomplete="code" />
                            @error('code')
                        ...
                        <div class="flex-grow mb-4 mr-2">
                            ...
                            <x-input id="nom" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="nom" value="{{ $niveauetude->nom }}" required autofocus autocomplete="nom" />
                            @error('nom')
                                <span class="text-xs text-red-500">
                                    {{$message}}
                                </span>
...
<div class="form-group">
 <label for="diplome_id">Sélectionner un diplome :</label>
    <select class="shadow appearance-none border-2 border-gray-400 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline enlever_shadow" id="diplome_id" name="diplome_id">
        ...
        @endforeach
                    ...
                    <div class="mb-4 mx-auto w-full">
                        ...
                        <textarea value="{{ $niveauetude->description }}" class="enlever_shadow shadow appearance-none border-2 border-gray-400 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:border-first-orange" id="description" name="description" required rows="4" placeholder="Description">{{$niveauetude->description}}</textarea>
                        @error('description')
                            <span class="text-xs text-red-500">
                                {{$message}}
                            </span>
