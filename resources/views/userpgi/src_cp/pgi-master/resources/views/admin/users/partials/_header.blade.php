        ...
        <div class="relative">
            <div class="inset-y-4 pt-2 left-0">
                <a href="{{ route('users.index') }}">
                    <button type="button" class="{{ $btnTous }}">
                        Tous ({{ $roleStats['total'] }})
                    </button>
                </a>
                @foreach($roleStats['items'] as $key=>$value)
                    <a href="{{ route('users.filter.role', strtolower($key)) }}">
                        <button type="button" class="{{ isset($role) && $role === $key ? $activeBtn : $normalBtn }}">
                            {{ $value['nom'] }} ({{ $value['nb'] }})
                        </button>
                    </a>
                @endforeach
