...
<body class="font-sans antialiased">
  ...
  <div class="mx-auto max-w-screen-lg pt-10 mb-[5%]">
    <h1 class="text-4xl text-first-orange pb-10">Toutes les actualités</h1>
    <div class="flex flex-wrap justify-center -mx-4">
    @foreach($news as $new)
        <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/3 xl:w-1/3 px-4 mb-8">
            <div class="p-32 rounded-md shadow-md bg-cover" style="background-image: url('{{ asset('storage/front/actualites/' . $new->actualite_main_pic) }}');">
                <h3 class="text-xl font-bold mb-2"></h3>
            </div>
            <p class="text-sm pt-2">{{ $new->updated_at->formatLocalized('%d %b') }}</p>
            <p class="text-base text-first-orange pt-1">{{$new->actualite_titre}}</p>
            <a class="text-xs text-first-orange pt-1" style="cursor: pointer" href="{{ route('news.show', ['id' => $new->id]) }}">Lire la suite</a>
        </div>
    @endforeach
</div>
