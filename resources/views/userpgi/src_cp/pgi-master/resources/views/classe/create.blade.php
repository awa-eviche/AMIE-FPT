<x-app-layout>
    ...
    <div class="rounded-sm w-full">
        ...
        <div class="mx-auto max-w-5xl shadow-xl rounded">
            <form action="{{ route('classe.store') }}" method="POST" class="bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                ...
                <div class="border border-gray-200 p-4">
                    <div class="flex flex-wrap w-full justify-evenly">
                        <div class="flex-grow  mb-4 mr-2">
                            ...
                            <x-input id="libelle" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" :value="old('libelle')" required autofocus autocomplete="libelle" />
                            @error('libelle')
                        ...
                        <div class="flex-grow mb-4 mr-2">
                            ...
                            <select class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" id="annee_academique_id" name="annee_academique_id">
                                ...
                                <option value="{{ $anneeacademique->id }}">{{ $anneeacademique->annee1 . '-' . $anneeacademique->annee2}}</option>
                                @endforeach
                            </select>
                    ...
                    @livewire('Classe.CreateClasse')



              
                            <x-input id="etablissement_id" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2" type="hidden" name="etablissement_id" :value="optional(auth()->user()->personnel)->etablissement_id" required autofocus autocomplete="etablissement_id" />
