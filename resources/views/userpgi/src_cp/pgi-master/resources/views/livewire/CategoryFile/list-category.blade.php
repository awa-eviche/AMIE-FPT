<div class="max-w-10xl mx-auto mt-5">
    <div class="w-full flex justify-between">
        ...
        @if ($typeUser === "DFPT")
            <div class="">
                ...
                <a href="{{ route('create.category') }}" class="mr-3 px-3 rounded-md py-3 flex text-white text-sm text-center bg-first-orange">
        ...
        <div  class=" w-full z-50 justify-center items-center h-full ">
            ...
            <div class=" bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 space-y-4">
                    <iframe class="w-full" src="{{ $path}}"></iframe>
                </div>
    ...
    {{-- Edite document --}}
        @if ($editDocument)
            <div class="mx-auto max-w-5xl shadow-xl rounded">
                <form action="{{ route('update.document', $idDocument)}}" method="POST" class="bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4" enctype="multipart/form-data">
                   @method('put') 
                    @csrf
                    <h3 class="bg-gray-100 p-2 text-sm font-bold text-first-orange">
                        Modifier le nom document :{{ $idDocument }}
                    </h3>
                    <div class="border border-gray-200 p-4">
                        <div class="flex flex-wrap w-full justify-between">
                            <div class="flex-grow mb-4 mr-2">
                                ...
                                <x-input id="libelle"  class=" w-96 focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" value="{{ $libelleDocument }}" required autofocus autocomplete="libelle" />
                                @error('libelle')
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <div class=" flex justify-evenly">
                        <form action="{{ route('delete.document', $idDocument) }}" method="POST">
                            @method('delete')
                            @csrf
    ...
    <div class="py-10">
        <div class="flex justify-start bg-white border-x-2 rounded px-8 pt-6 pb-8 mb-4">
            <div class="w-1/4  text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                @forelse ($categoryFiles as $categoryFile)
                <div class="flex  justify-start ">
                        <a  class="element_sidebar selectCategory"  wire:click="getFileCategory({{ $categoryFile->fichiers }}, {{ collect( $categoryFile->access) }})">
                            <span class="text-left">
                                <i class="menu-icon fa-regular fa-folder-open fa-sm fa-fw"></i>
                            </span>
                            <span class="mx-2 text-sm font-bold ">
                                {{ $categoryFile->libelle }} ({{ $categoryFile->fichiers->count() }})
                            </span>
                            
                    </a>
                    <div class="flex items-start ">
                        <button id="dropdownMenuIconButton" data-dropdown-toggle="{{ $categoryFile->id }}" data-dropdown-placement="bottom-start" class="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800 dark:focus:ring-gray-600" type="button">
                            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 4 15">
                            <path d="M3.5 1.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 6.041a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 5.959a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"/>
                            </svg>
                        </button>
                        
                        <div id="{{ $categoryFile->id }}" class="z-10 my-48 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-40 dark:bg-gray-700 dark:divide-gray-600">
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                                @if ($typeUser === "DFPT" || $categoryFile->access??[$typeUser][1] =="Ajouter")
                                    <li>
                                        <a href="{{ route('show.category', $categoryFile->id) }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            Ajouter un document
                                        </a>
                                    </li>  
                                @endif
                                @if ($typeUser == "DFPT" || $accessUpdate ==="Modifier"  )
                                    <li>
                                        <a href="{{ route('edite.category', $categoryFile->id) }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            Modifier
                                        </a>
                                    </li>
                                ...
                                @if ($typeUser == "DFPT" )
                                    <li>
                                        <a href="#"  data-modal-target="'{{ $categoryFile->id }}'" data-modal-toggle="'{{ $categoryFile->id }}'" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            Supprimer
                                        </a>
                                    </li>
                                @endif
                ...
                <div id="'{{ $categoryFile->id }}'" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="'{{ $categoryFile->id }}'">
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                </svg>
                                <span class="sr-only">Close modal</span>
                            ...
                            <div class="p-4 md:p-5 text-center">
                                ...
                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Voulez vous vraiment suprimer la catégirie:{{ $categoryFile->libelle }} </h3>
                                <div class=" flex justify-evenly">
                                    <form action="{{ route('delete.category',  $categoryFile->id) }}" method="POST">
                                        @method('delete')
                                        @csrf
                                        <button  type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                        Oui
                ...
                @if ($fichiers)
                    @forelse ($fichiers as $fichier)
                        <div  class="min-w-35 h-48 m-3 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700" type="button">
                                ...
                                <div class=" flex justify-evenly">
                                    <a href="#" wire:click="getpath('{{ Storage::url($fichier["document"]) }}', '{{ $fichier["libelle"]}}')">
                                        <span class="text-left">
                                            <i class="fas fa-eye fa-sm fa-fw" aria-hidden="true"></i>
                                        </span>
                                    </a>
                                    <a href="#" wire:click="download('{{ $fichier["document"] }}', '{{ $fichier["libelle"]}}')">
                                        <i class="fas fa-download fa-sm fa-fw"></i>
                                    </a>
                                        
                                        @if ($typeUser === "superadmin" || $fichier['user_id'] == auth()->user()->id || $accessUpdate ==="Modifier" )
                                            <a href="#" wire:click="edit({{ $fichier['id'] }},'{{ $fichier['libelle'] }}')">
                                                <i class="fas fa-edit fa-sm fa-fw" aria-hidden="true"></i>
                                            </a>
                        
                                        @endif
                                        @if ($typeUser === "superadmin" || $fichier['user_id'] == auth()->user()->id ||$accessDelete ==="Supprimer")
                                        
                                            <a href="#" wire:click="delete({{ $fichier['id'] }})">
