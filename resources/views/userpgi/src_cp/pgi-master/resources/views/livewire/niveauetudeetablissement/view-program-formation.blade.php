<div>
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            @if($approved == false)
            <a href="#" wire:click="validateProgram" class="px-4 rounded-md py-4 flex text-white text-xs font-bold text-center bg-blue-500 items-center hover:bg-blue-700">
                <span class="mx-2">Valider le programme</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 m-2">
        @foreach($les_metiers as $nom => $niveaux)
        <div class="w-full h-auto bg-white rounded-md shadow-md">
            <div class="p-3">
                <div class="font-bold text-lg mb-2 bg-gray-200 p-2 rounded">{{ Str::limit($nom ?? '-', 32) }}</div>
            </div>
            <div class="px-3 py-2">
                <ul class="max-h-48 overflow-y-auto">
                    @foreach($niveaux as $niv)
                    <li class="border-b p-2 font-normal mb-1">
                        <i class="fa fa-circle text-xs me-2"></i>{{ $niv->nom ?? '-' }}
                    </li>
                    @endforeach
                </ul>
            </div>
