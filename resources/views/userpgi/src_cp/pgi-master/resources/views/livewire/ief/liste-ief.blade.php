<div class="pr-5">
    <div class="flex mb-5 justify-between">
        <div class="flex">
            <span href="#" class="bg-transparent border-transparent px-4  py-2 flex text-black text-sm text-center  bg-white items-center">
                <svg class="w-6 h-6 text-first-orange font-bold" arief-hidden="true" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="flex">
            ...
            <a href="{{route('ief.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter une ief</span>
    ...
    <div class="w-full rounded-lg shadow-xs p-0">
            ...
            <table class="w-full border-t mb-3">
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    <tr class="text-gray-700 ">
                        ...
                        <td class="px-6 py-3 border-b">{{ $ief->nom ?? ' - ' }}</td>

                        <td class="px-6 py-3 border-b">{{ $ief->email ?? '-' }}</td>

                        <td class="px-6 py-3 border-b">{{ $ief->telephone ?? '-' }}</td>

                        <td class="px-6 py-3 border-b">{{ $ief->adresse ?? '-' }}</td>

                        <td class="px-6 py-3 border-b">{{ $ief->ia->nom ?? '-' }}</td>

                        <td class="px-6 py-3 border-b flex">
                            <div class="relative" x-data="{ open: false }" @click.away="open = false" @close.stop="open = false">
                                <div @click="open = ! open">
                                    <button class="bg-dark hover:first-orange text-white font-semibold py-2 px-6 rounded inline-flex items-center justify-end">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute z-50 mt-1 w-48 rounded-md shadow-lg origin-top-right right-0" @click="open = false" style="display: none;">
                                    <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                        <div class="border shadow-md py-2 text-center">
                                            <a href="{{ route('ief.show', $ief->id) }}" class="text-purple-600">Voir
                                            </a>
                                        </div>
                                        <div class="border shadow-md py-2 text-center">
                                            <a href="{{ route('ief.edit', $ief->id) }}" class="text-purple-600">
                                                Modifier
                                            </a>
                                        </div>
                                        <div class="border shadow-md py-2 text-center">
                                            ...
                                            'route' => array('ief.destroy', $ief->id))) !!}
                                            {{ csrf_field() }}
                            ...
                            <form class="text-center" action="{{ route('ief.destroy', $ief->id) }}" method="POST">
        ...
        @empty
        <tr>
            <td colspan="6" class="px-6 py-4 font-bold text-lg text-center">Aucune donnée disponible</td>
        </tr>
        @endforelse
        </tbody>
        </table>
