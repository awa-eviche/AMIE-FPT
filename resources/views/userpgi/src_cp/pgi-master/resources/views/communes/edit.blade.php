<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification d\'une commune') }}
        </h2>
    </x-slot>
    <div class="flex mb-4 text-sm font-bold p-3 bg-white">
        <p>
            <a href="{{route('commune.index')}}"  class="text-maquette">Accueil</a>
              <span class="mx-2 text-maquette">/</span>
            <a href="{{route('commune.index')}}"  class="text-maquette">Référentiel</a>
              <span class="mx-2 text-maquette">/</span>
        <p> <a href="{{route('commune.index')}}" >commune  </a>
...
    </div>
    ...
    <div class="rounded-sm w-full">
        <div class="w-full mx-auto max-w-6xl">
            ...
            <form class="bg-white shadow-md rounded pt-6 pb-8 mb-4" action="{{ route('commune.update', $commune->id) }}" method="POST">
                @csrf
                ...
                <div class=" bg-white  w-full rounded-sm" style="height:200%;">
                    ...
                    <div class="max-w-7xl sm:px-2 lg:px-4 ">
                            ...
                            <div class="flex flex-wrap w-full justify-evenly">
                                <div class="flex-grow mb-4 mr-2">
                                    ...
                                    <x-input id="code" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="code" value="{{ $commune->code }}" required autofocus autocomplete="code" />
                                    @error('code')
                                ...
                                <div class="flex-grow mb-4 mr-2">
                                    ...
                                    <x-input id="libelle" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " type="text" name="libelle" value="{{ $commune->libelle}}" required autofocus autocomplete="libelle" />
                                    @error('libelle')
                                        <span class="text-xs text-red-500">
                                            {{$message}}
                    ...
                    <div class="flex-grow mb-4 mr-2">
                        ...
                        <select class="shadow appearance-none border-2 border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="departement_id" name="departement_id" required>
                            <option value="{{$commune->departement_id}}"><span class="text-red-500">{{$commune->departement->libelle}}</span></option>
