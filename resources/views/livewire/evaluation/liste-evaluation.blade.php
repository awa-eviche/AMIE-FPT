<div class="pr-5">
    <div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            @if(auth()->user()->can('edit_inscription') and !auth()->user()->hasRole(config('constants.roles.superadmin')))
            <a href="{{route('inscription.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
    ...
    <div class="flex  -mx-3 mb-0  space-x-4 space-x-reverse" style="padding:8px;">
        <div class="w-full px-7">
            ...
            <select id="annee_academique" wire:model="selectedInscriptionAnnee" name="selectedInscriptionAnnee" wire:change="$refresh" class="block w-full focus:border-first-orange enlever_shadow rounded px-2 py-0.75 shadow-first-orange text-sm border-2 " required autofocus>
                ...
                @foreach ($annee_academique as $annee_academiques)
                <option value="{{$annee_academiques->id}}">{{ $annee_academiques->dateDebut}}-{{$annee_academiques->dateFin}}</option>
                @endforeach
...
<tr class="text-gray-700 dark:text-gray-400">
    <td class="px-4 py-3">
        {{ $evaluation->inscription->apprenant->nom ?? '-' }} {{ $evaluation->inscription->apprenant->prenom ?? '-' }} / {{ $evaluation->inscription->apprenant->telephone ?? '-' }}
    </td>
    <td class="px-4 py-3 text-xs font-bold">{{ $evaluation->inscription->classe->libelle ?? '-' }}</td>
    <td class="px-4 py-3 text-xs font-bold">{{ $evaluation->inscription->classe->niveau_etude->metier->nom ?? '-' }}</td>
    <td class="px-4 py-3 text-xs font-bold">{{ $evaluation->matiere->nom ?? '-' }}</td>
    <td class="px-4 py-3 text-xs font-bold">{{ $evaluation->note_cc_semestre1 ?? '-' }}</td>
<td class="px-4 py-3 text-xs font-bold">{{ $evaluation->note_composition_semestre1 ?? 'Non disponibles' }}</td>
<td class="px-4 py-3 text-xs font-bold">{{ $evaluation->note_cc_semestre2 ?? 'Non disponibles' }}</td>
<td class="px-4 py-3 text-xs font-bold">{{ $evaluation->note_composition_semestre2 ?? ' disponibles' }}</td>

    <td class="px-4 py-3 text-xs">
        @if ($evaluation->inscription && $evaluation->matiere)
            <a href="{{ route('evaluation.edit', ['inscriptionId' => $evaluation->inscription->id, 'matiereId' => $evaluation->matiere->id]) }}">
...
<button class="text-red-500 mr-2 apix-delete" data-id="{{ $evaluation->id }}" data-toggle="tooltip" title="Supprimer">
