...
<div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('critere.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
    ...
    <form action="{{ route('critere.index') }}" method="GET">
    ...
    <div class="flex mb-5 gap-4">
        ...
        <div>
            ...
            <select wire:model="selectedNiveau" id="niveau" wire:change="$refresh"
                    class="border border-gray-300 p-2 rounded text-sm" {{ empty($niveaux) ? 'disabled' : '' }}>
        ...
        <div>
            ...
            <select wire:model="selectedCompetence" id="competence" wire:change="$refresh"
                    class="border border-gray-300 p-2 rounded text-sm" {{ empty($competences) ? 'disabled' : '' }}>
        ...
        <div>
            ...
            <select wire:model="selectedElement" id="element"
                    class="border border-gray-300 p-2 rounded text-sm" {{ empty($elements) ? 'disabled' : '' }}>
...
@if($count)
    <div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} critères trouvés</span></div>
@endif
        ...
        <div class="text-sm w-full p-0">
            ...
            <thead>
                    ...
                    @forelse ($criteres as $critere)
                        ...
                        <td class="px-4 py-3 border-b">{{ $critere->competence->nom ?? ' - ' }}</td>
                            <td class="px-4 py-3 border-b">{{ $critere->elementCompetence->nom ?? ' - ' }}</td>
                         
                            <td class="px-4 py-3 border-b">{{ $critere->libelle ?? ' - ' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('critere.show', $critere->id) }}"
                                                    class="text-purple-600">Visualiser
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('critere.edit', $critere->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
