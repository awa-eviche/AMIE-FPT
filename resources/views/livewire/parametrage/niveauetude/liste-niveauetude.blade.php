...
<div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('niveauetude.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter un niveau d'etude</span>
    ...
    <form action="{{ route('niveauetude.index') }}" method="GET">
     
<div style="display: inline-block;margin-right: 20px; ">
         <label for="métier" style="text-align:center;">Filtrer par métier :</label><br>
        <select  class="block  focus:border-first-orange enlever_shadow rounded  shadow-first-orange text-sm border-2" name="metier_id"wire:model="selectedNiveauMetier" id="selectedNiveauMetier" wire:change="$refresh">
            <option value="">Sélectionner un métier</option> 
             @foreach($metiers as $metier)
        <option value="{{$metier->id }}">{{$metier->nom }}</option>
          @endforeach
            <!-- Ajoutez d'autres options de filières si nécessaire -->
        </select>
    </div>
    <div style="display: inline-block;">
         <label for="niveau"style="text-align:center;">Filtrer par diplôme :</label><br>
        <select  class="block  focus:border-first-orange enlever_shadow rounded  shadow-first-orange text-sm border-2" name="niveau_id"wire:model="selectedNiveauDiplome" id="selectedNiveauDiplome" name="niveauetude_id" wire:change="$refresh">
 ...
 <br>
    @if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} Niveaux d'études trouvés</span></div>
@endif
        ...
        <div class="text-sm w-full p-0">
            ...
            <thead>
                                    ...
                                    <th class="px-4 py-4">Code</th>
                                        ...
                                        <th class="px-4 py-4 text-white font-norma">Metier</th>
                                        <th class="px-4 py-4 text-white font-norma ">Diplôme</th>
                ...
                <tbody class="bg-white divide-y ">
                    @php
                        $num = 1;
                    @endphp
                    @forelse ($niveauetudes as $niv)
                        <tr class="text-gray-700 ">
                            
                       
                                    <td class="px-6 py-3 border-b">{{ $num}}</td> 
                            <td class="px-6 py-3 border-b">{{ $niv->code ?? ' - ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $niv->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $niv->metier->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $niv->diplome->nom ?? '-' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('niveauetude.show', $niv->id) }}"
                                                    class="text-purple-600">Voir
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('niveauetude.edit', $niv->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('niveauetude.destroy', $niv->id))) !!}
                    {{ csrf_field() }}
...
@endif
                                ...
                                {{--  <div
                                    ...
                                    <a href="{{ route('niveauetude.edit', $niveauetude->id) }}" class="text-purple-600">
                                    ...
                                    <form class="text-center"
                                        action="{{ route('niveauetude.destroy', $niv->id) }}" method="POST">
