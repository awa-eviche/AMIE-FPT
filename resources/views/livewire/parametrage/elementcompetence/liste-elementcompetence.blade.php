...
<div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            @can('gerer_parametrage')
            <a href="{{( route('elementcompetence.multiple.create'))}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter un élément de compétence</span>
...
<div style="display: inline-block; margin-right: 10px;">
    <label for="selectedElementCompetenceMetier">Filtrer par métier :</label><br>
    <select name="metier_id" id="selectedElementCompetenceMetier"  wire:change="$refresh" wire:model="selectedElementCompetenceMetier">
...
{{-- Sélection du niveau (affiché seulement si un métier est choisi) --}}
@if(!empty($niveaux) && $niveaux->count())
<div style="display: inline-block; margin-right: 10px;">
    <label for="selectedElementCompetenceNiveau">Filtrer par niveau :</label><br>
    <select name="niveau_id" id="selectedElementCompetenceNiveau" wire:change="$refresh"  wire:model="selectedElementCompetenceNiveau">
...
{{-- Sélection de la compétence (selon métier et niveau) --}}
<div style="display: inline-block;">
    <label for="selectedElementCompetenceMatiere">Filtrer par compétence :</label><br>
    <select name="competence_id" id="selectedElementCompetenceMatiere" wire:change="$refresh"  wire:model="selectedElementCompetenceMatiere">
...
@if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} éléments de compétences trouvés</span></div>
@endif
    <div class="w-full rounded-lg shadow-xs p-0">
        <div class="text-sm w-full p-0">
            <table class="w-full border-t mb-3">
            <thead>
                                    <tr class="text-xs font-bold tracking-wide text-left text-maquette-gris uppercase border-b bg-first-orange">
                                    <th class="px-6 py-3">N° </th> 
                                     
                                    <th class="px-4 py-4">Code</th>
                                        <th class="px-4 py-4">Libellé</th>
                                        <th class="px-4 py-4 text-white font-bold">Metier</th>
                                        <th class="px-4 py-4 text-white font-bold">Compétence</th>
                                        <th class="px-2 py-4 w-[60px]">Actions</th>
                                    </tr>
                                </thead>
                ...
                <tbody class="bg-white divide-y ">
                    @php
                        $num = 1;
                    @endphp
                    @forelse ($elementcompetences as $elementcompetence)
                        <tr class="text-gray-700 ">
                      
                                    <td class="px-6 py-3 border-b">{{ $num}}</td> 
                         
                            <td class="px-6 py-3 border-b">{{ $elementcompetence->code ?? ' - ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $elementcompetence->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $elementcompetence->metier->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $elementcompetence->competence->nom ?? '-' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('elementcompetence.show', $elementcompetence->id) }}"
                                                    class="text-purple-600">Voir
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('elementcompetence.edit', $elementcompetence->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('elementcompetence.destroy', $elementcompetence->id))) !!}
                    {{ csrf_field() }}
...
@endif
                                ...
                                {{--  <div
                                    ...
                                    <a href="{{ route('matiere.edit', $matiere->id) }}" class="text-purple-600">
                                    ...
                                    <form class="text-center"
                                        action="{{ route('elementcompetence.destroy', $elementcompetence->id) }}" method="POST">
        ...
        <div class="flex justify-start items-center mt-5">
                <button {{$startLimit == 0 ? 'disabled' : '' }} wire:click="prev" type="button" class="bg-white px-6 rounded-md border-white py-3 flex text-black text-sm text-center shadow-lg items-center">
                ...
                <button wire:click="next" {{($startLimit+10) >= $count ? 'disabled' : '' }} type="button" class="bg-white px-6 rounded-md border-white py-3 flex text-black text-sm text-center shadow-lg items-center">
