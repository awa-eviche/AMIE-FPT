...
<div class="flex mb-5 justify-between">
        ...
        <a href="#" class="mx-2 px-5 rounded-md py-0 flex text-orange-400 text-xs font-bold text-center shadow-md bg-white items-center">
            ...
            <a href="{{route('metier.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter un metier</span>
    ...
    <label for="filiere" style="text-align:center;">Filtrer par filiére :</label><br>
        <select  class="block  focus:border-first-orange enlever_shadow rounded  shadow-first-orange text-sm border-2" name="secteur_id" wire:model="selectedMetierFiliere" id="selectedMetierFiliere" wire:change="$refresh">
...
<br>
@if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} métiers trouvés</span></div>
@endif
    ...
    <div class="w-full rounded-lg shadow-xs p-0">
        <div class="text-sm w-full p-0">
            ...
            <tr class="text-xs font-black tracking-wide text-left text-maquette-gris font-bold uppercase border-b bg-first-orange">
                                       ...
                                       <th class="px-4 py-4 text-white font-normal" >Code</th>
                                        <th class="px-4 py-4 text-white font-normal">Libellé</th>
                                       
                                        <th class="px-4 py-4 text-white font-bold">Filiere</th>
                                        <th class="px-2 py-4 w-[60px] text-white font-normal">Actions</th>
                    </tr>
                                </thead>
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    @forelse ($metiers as $metier)
                        <tr class="text-gray-700 ">
                            ...
                            <td class="px-6 py-3 border-b">{{ $metier->code ?? ' - ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $metier->nom ?? '-' }}</td>
                            
                            <td class="px-4 py-3 border-b">{{ $metier->filiere->nom ?? '-' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('metier.show', $metier->id) }}"
                                                    class="text-purple-600">Voir
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('metier.edit', $metier->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('metier.destroy', $metier->id))) !!}
                    {{ csrf_field() }}
...
@endif
                                ...
                                {{--  <div
                                    ...
                                    <form class="text-center"
                                        action="{{ route('metier.destroy', $metier->id) }}" method="POST">
