...
<div class="flex mb-5 justify-between">
 ...
 @can('gerer_parametrage')
            <a href="{{route('matiere.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter une matiere</span>
...
<div style="display: inline-block;margin-right: 10px;">
        ...
        <select name="metier_id"wire:model="selectedMatiereMetier" id="selectedMatiereMetier" wire:change="$refresh">
    ...
    <div style="display: inline-block;">
        ...
        <select name="niveau_id"wire:model="selectedMatiereNiveau" id="selectedMatiereNiveau" name="niveauetude_id" wire:change="$refresh">
 ...
 @if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} matiéres trouvées</span></div>
@endif
 ...
 <th class="px-4 py-4 text-white font-bold">Coefficient</th>
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    @forelse ($matieres as $matiere)
                        <tr class="text-gray-700 ">
                            ...
                            <td class="px-6 py-3 border-b">{{ $matiere->code ?? ' - ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $matiere->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $matiere->metier->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $matiere->niveau_etude->nom ?? '-' }}</td>
       <td class="px-6 py-3 border-b">{{ $matiere->coef ?? '-' }}</td>
 ...
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('matiere.destroy', $matiere->id))) !!}
                    {{ csrf_field() }}
...
@endif
                                ...
                                {{--  <div
                                    ...
                                    <form class="text-center"
                                        action="{{ route('matiere.destroy', $matiere->id) }}" method="POST">
