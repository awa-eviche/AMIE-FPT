...
<div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('filiere.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter une filiere</span>
    ...
    <label for="secteur" style="text-align:center;">Filtrer par secteur :</label><br>
        <select  class="block  focus:border-first-orange enlever_shadow rounded  shadow-first-orange text-sm border-2 "name="filiere_id" wire:model="selectedFiliereSecteur" id="selectedFiliereSecteur" wire:change="$refresh">
...
<br>
@if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} filiéres trouvées</span></div>
@endif
        ...
        <div class="text-sm w-full p-0">
            ...
            <thead>
                                    ...
                                    <th class="px-4 py-4">Code</th>
                                        ...
                                        <th class="px-4 py-4 text-white font-bold">Secteur</th>
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    @forelse ($filieres as $filiere)
                        ...
                        <td class="px-6 py-3 border-b">{{ $num }}</td>
                            <td class="px-6 py-3 border-b">{{ $filiere->code ?? ' - ' }}</td>
                
                            <td class="px-6 py-3 border-b">{{ $filiere->nom ?? '-' }}</td>
                            
                            <td class="px-4 py-3 border-b">{{ $filiere->secteur->libelle ?? '-' }}</td>
                                ...
                                <div class="relative" x-data="{ open: false }" @click.away="open = false"
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            ...
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('filiere.edit', $filiere->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('filiere.destroy', $filiere->id))) !!}
                    {{ csrf_field() }}
 ...
 @endif
                                ...
                                {{--  <div
                                    ...
                                    <form class="text-center"
                                        action="{{ route('filiere.destroy', $filiere->id) }}" method="POST">
