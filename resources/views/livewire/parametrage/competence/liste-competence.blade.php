...
<div class="flex mb-5 justify-between">
        ...
        <div class="flex">
            ...
            <a href="{{route('competence.create')}}" class="px-3 rounded-md py-3 flex text-white text-xs font-bold text-center bg-orange-400 items-center">
                ...
                </svg><span class="mx-2">Ajouter une competence</span>
    ...
    <form action="{{ route('competence.index') }}" method="GET">
    
<div style="display: inline-block;margin-right: 10px;">
         <label for="métier" style="text-align:center;">Filtrer par métier :</label><br>
        <select name="metier_id"wire:model="selectedCompetenceMetier" id="selectedCompetenceMetier" wire:change="$refresh">
    ...
    <div style="display: inline-block;">
         <label for="niveau"style="text-align:center;">Filtrer par niveau :</label><br>
        <select name="niveau_id"wire:model="selectedCompetenceNiveau" id="selectedCompetenceNiveau" name="niveauetude_id" wire:change="$refresh">
...
@if($count)
<div class="mb-2"><span class="bg-green-100 font-bold p-1 px-2 rounded">{{ $count }} compétences trouvés</span></div>
@endif
        ...
        <div class="text-sm w-full p-0">
            ...
            <thead>
                                    ...
                                    <th class="px-4 py-4">Code</th>
                                     <th class="px-4 py-4">Type de Compétence</th>
                                        <th class="px-4 py-4">Libellé</th>
                                        <th class="px-4 py-4 text-white font-bold">Metier</th>
                                        <th class="px-4 py-4 text-white font-bold">Niveau Etude</th>
                ...
                <tbody class="bg-white divide-y ">
                    ...
                    @forelse ($competences as $competence)
                        <tr class="text-gray-700 ">
                            ...
                            <td class="px-6 py-3 border-b">{{ $competence->code ?? ' - ' }}</td>
                           <td class="px-6 py-3 border-b">{{ $competence->type ?? ' - '  }}</td>
                            <td class="px-6 py-3 border-b">{{ $competence->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $competence->metier->nom ?? '-' }}</td>
                            <td class="px-4 py-3 border-b">{{ $competence->niveau_etude->nom ?? '-' }}</td>
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                        ...
                                        <div class="rounded-md ring-1 ring-black ring-opacity-5 bg-white">
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('competence.show', $competence->id) }}"
                                                    class="text-purple-600">Voir
                                                </a>
                                            </div>
                                            <div class="border shadow-md py-2 text-center">
                                                <a href="{{ route('competence.edit', $competence->id) }}" class="text-purple-600">
                                                    Modifier
                                                </a>
                                            </div>
 @if(auth()->user()->hasRole(config('constants.roles.superadmin')))
                    ...
                    'route' => array('competence.destroy', $competence->id))) !!}
                    {{ csrf_field() }}
...
@endif
                                ...
                                {{--  <div
                                    ...
                                    <form class="text-center"
                                        action="{{ route('competence.destroy', $competence->id) }}" method="POST">
