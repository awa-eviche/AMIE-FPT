...
class ApprenantsImport implements OnEachRow, WithHeadingRow, SkipsOnError, SkipsOnFailure
{
    ...
    protected $classe;
    protected $anneeAcademiqueId;

    public function __construct($classe, $anneeAcademiqueId)
    {
        $this->classe = $classe;
...
if (isset($data['sexe'])) {
            ...
            if (in_array($sexe, ['m', 'masculin'])) {
                $data['sexe'] = 'M';
            } elseif (in_array($sexe, ['f', 'feminin', 'féminin'])) {
                $data['sexe'] = 'F';
            } else {
                $data['sexe'] = null; // ou laisse tel quel pour que la validation échoue
            }
        }
        $validator = Validator::make($data, [
            ...
            'telephone' =>  'required|unique:apprenants,telephone',
     ...
     private function convertDate($value, $rowIndex, $fieldName)
{
    ...
    try {
        // Cas où c'est un nombre Excel
        if (is_numeric($value)) {
            return Date::excelToDateTimeObject($value)->format('Y-m-d');
        }

        // Nettoyage des espaces invisibles
        $clean = trim(str_replace("\xC2\xA0", '', $value));

        // Essayer avec année 4 chiffres
        try {
            return Carbon::createFromFormat('d/m/Y', $clean)->format('Y-m-d');
        } catch (\Exception $e) {
            // Essayer avec année 2 chiffres
            return Carbon::createFromFormat('d/m/y', $clean)->format('Y-m-d');
        }
