...
{
    $nv = NiveauEtudeEtablissement::where('niveau_etude_id', $id)
        ->where('etablissement_id', $idEtablissement)
        ->first();

    if (!$nv) {
        return redirect()->back()->with('error', 'Aucun programme trouvé pour cet établissement.');
    }
    ...
    return redirect()
        ->route('program.view', $idEtablissement)
        ->with('success', 'Le programme de formation a été supprimé définitivement.');
}
